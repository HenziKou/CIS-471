...
def betterEvaluationFunction(currentGameState):
    ...
    # 1
    d_ghost = 999999999999999999999999  # Ghost distance
    n_ghost = currentGameState.getNumAgents() - 1   # Number of ghosts
    val = 1

    while(val <= n_ghost):
        n_dist = manhattanDistance(currentGameState.getPacmanPosition(), currentGameState.getGhostPosition(val))
        if(n_dist < d_ghost):
            d_ghost = n_dist
        val += 1
    ...
    closest_f = 999999999999999999999999    # Closest food

    for food in food_list:
        dist = manhattanDistance(food, currentGameState.getPacmanPosition())
        if(dist < closest_f):
            closest_f = dist
    value -= closest_f * 1.5
